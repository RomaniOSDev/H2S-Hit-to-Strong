...
extension RouteStringCipher {
    static var bootstrapCaption: String { reveal([235, 200, 198, 195, 206, 201, 192, 137, 137, 137]) }
}

struct DualArcSpinnerRing: View {
    var progress: Double
...
struct BootstrapCompletionGlyph: View {
...
struct BootstrapSplashGate: View {
    var body: some View {
        ZStack {
            Image(.loadingLogo)
                .resizable()
            ...
            VStack(spacing: 24) {
                ...
                DualArcSpinnerRing(progress: 0)

                Text(RouteStringCipher.bootstrapCaption)
...
typealias StartMainView = BootstrapSplashGate
typealias NewLoadTwoCircleView = DualArcSpinnerRing
typealias EndLoadingIndicator = BootstrapCompletionGlyph
...
private protocol BootstrapAnimationDriver {
    func driveCycle()
}

private enum BootstrapVisualState {
    case spinning
    case done
}

private struct BootstrapAnimationStub: BootstrapAnimationDriver {
    func driveCycle() {}
}
